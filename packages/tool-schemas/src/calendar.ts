...
export const calendarFindAvailabilitySchema = z.object({
  ...
  durationMinutes: z.number().int().min(15).max(480).default(30).describe('Required meeting duration in minutes'),
  attendeeEmails: z.array(z.string().email()).optional().default([]).describe('Optional attendee emails to check free/busy for'),
});
...
export const calendarListEventsSchema = z.object({
  ...
  maxResults: z.number().int().min(1).max(50).default(10).describe('Max events to fetch'),
...
export const calendarCreateEventSchema = z.object({
  ...
  attendees: z.array(z.string().email()).optional().default([]).describe('List of attendee email addresses'),
...
export const calendarUpdateEventSchema = z.object({
  ...
  attendees: z.array(z.string().email()).optional().describe('Updated attendees list'),
